...
def crop_axis_aligned(
    ...
    max_x = min(img.shape[1] - 1, int(np.max(pts[:, 0])) + margin)
    max_y = min(img.shape[0] - 1, int(np.max(pts[:, 1])) + margin)

    crop = img[min_y:max_y + 1, min_x:max_x + 1].copy()
    if crop.size == 0:
        return np.zeros((out_h, out_h, 3), dtype=np.uint8)
...
def preprocess_crop(img: np.ndarray, mode: str = "clahe_gamma_sharpen") -> np.ndarray:
    if mode == "none":
        return img.copy()
    elif mode == "clahe":
        return apply_clahe(img)
    elif mode == "gamma":
        return apply_gamma(img, gamma=1.2)
    elif mode == "sharpen":
        return apply_sharpen(img)
    elif mode == "clahe_sharpen":
        x = apply_clahe(img)
        x = apply_sharpen(x)
        return x
    elif mode == "clahe_gamma_sharpen":
        x = apply_clahe(img)
        x = apply_gamma(x, gamma=1.2)
        x = apply_sharpen(x)
        return x
    else:
...
def evaluate(
    ...
    crop_height: int = 64,
    crop_mode: str = "warp",
    preprocess_mode: str = "clahe_gamma_sharpen",
    min_w: float = 8.0,
    min_h: float = 8.0,
    min_area: float = 80.0,
    nms_iou_thr: float = 0.4,
    ...
    for img_name in image_names:
        ...
        total_pred_before += raw_pred_count
        ...
        for match_idx, (pi, gi, iou) in enumerate(matches):
            ...
            crop_for_ocr = preprocess_crop(crop, preprocess_mode)
            pred_text = run_ocr_vietocr(crop_for_ocr, ocr_model)
            ...
            if debug_vis is not None:
                ...
                crop_path = os.path.join(
                    save_debug_dir,
                    f"{os.path.splitext(img_name)[0]}_match{match_idx}_{crop_mode}_{preprocess_mode}_crop.png"
                )
        ...
        per_image_results.append({
            "image": img_name,
            "num_pred_before_refine": raw_pred_count,
        ...
        print(
            f"[{img_name}] pred_before={raw_pred_count} | pred_after={len(pred_polys)} "
            f"| gt_valid={len(valid_gt_items)} | matched={len(matches)} "
            f"| CER={np.mean(image_cers) if image_cers else 'NA'} "
...
def main():
    parser = argparse.ArgumentParser(
        description="Phase 4: Phase 3 + preprocessing with instance-level evaluation"
    )
    ...
    parser.add_argument("--vietocr_config", default="vgg_transformer", help="VietOCR config name")
    parser.add_argument("--crop_margin", type=int, default=4, help="Extra margin around crop")
    parser.add_argument("--crop_height", type=int, default=64, help="Output crop height")
    parser.add_argument("--crop_mode", choices=["axis", "warp"], default="warp",
                        help="axis = no rectification, warp = rectification")
    ...
    parser.add_argument(
        ...
        default="clahe_gamma_sharpen",
        help="Preprocessing mode before OCR"
    )
