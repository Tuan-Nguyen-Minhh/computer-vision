...
def evaluate(
        ...
        for match_idx, (pi, gi, iou) in enumerate(matches):
            ...
            if debug_vis is not None:
                ...
                crop_path = os.path.join(
                    save_debug_dir,
                    f"{os.path.splitext(img_name)[0]}_match{match_idx}_{crop_mode}_crop.png"
                )
    ...
    summary = {
        ...
        "crop_mode": crop_mode,
        "preprocess": False,
        "phase": "phase3_rectification_ablation",
        "box_postprocess": {
            "conf_thr": conf_thr,
            "min_w": min_w,
            "min_h": min_h,
...
def main():
    parser = argparse.ArgumentParser(
        description="Phase 3: Phase 2 + rectification ablation (axis vs warp) with instance-level evaluation"
    )
